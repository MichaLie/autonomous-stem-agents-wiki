...
def main(release: bool = False) -> int:
    ...
    for index, record in enumerate(records):
        ...
        if profile == "foundation-models":
            ...
            if (autonomy == "B") != (category == "benchmark"):
                errors.append(f"{where}: B is reserved for category=benchmark and every benchmark must use B")
            if record.get("access") == "source-available" and not record.get("access_evidence"):
                errors.append(f"{where}.access_evidence: source-available records require licence evidence")
    ...
    deprecated_path = Path("deprecated_ids.json")
    if deprecated_path.is_file():
        ...
        for index, entry in enumerate(deprecated):
            ...
            if replacement_id is not None and replacement_id not in active_ids:
                errors.append(
                    f"{deprecated_path}[{index}].replacement_id: does not resolve to an active stable ID"
                )
    ...
    if profile == "autonomous-agents":
        pairs.extend([
            (Path("autonomous_stem_agents_wiki.md"), docs / "autonomous_stem_agents_wiki.md"),
            (Path("deprecated_ids.json"), docs / "deprecated_ids.json"),
            (Path("deprecated_ids.schema.json"), docs / "deprecated_ids.schema.json"),
        ])
